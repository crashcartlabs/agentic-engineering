...
cases, edge cases, non-goals, and product constraints. The `plan` skill later decides **how
...
- **Is:** a foreground conversation that produces one approved Markdown product spec
  under `specs/`.
- **Is:** the product contract that the `plan` skill reads before technical planning.
...
stop and recommend the `wayfinder` skill instead of fabricating a spec.
...
Use the `domain-modeling` skill's style discipline: challenge vague terms, overloaded words, and
...
- Do not choose exact test seams. You may state acceptance surfaces; the `plan` skill chooses
...
- Can the `plan` skill read this and know what product behavior must be preserved?
...
- Is implementation detail deferred to the `plan` skill?
...
- Route to the `wayfinder` skill when the destination is still too foggy to specify.
