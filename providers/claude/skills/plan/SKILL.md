---
name: plan
description: "Interactive planning skill — grills you to alignment, then writes a structured Markdown implementation plan that an executor agent can follow and a reviewer agent can check against. Explicit-trigger only; invoke the `plan` skill and never auto-start."
...
**Mark each phase's TDD discipline.** Every implementation phase carries a `TDD:` line next to its Validation line: `TDD: strict` for a behavioral phase the executor must build through the `tdd` red-green loop, or `TDD: none — <reason>` (scaffolding, config, generated files, pure refactor under existing tests). This is what arms the `tdd` skill's "an approved plan marks a phase as strict TDD" trigger — an unmarked phase leaves the executor guessing the testing contract.

### Gate 2 — Plan approved

Present the written plan. The human reviews and approves it as the **contract** before it ever reaches an executor. Revise until approved. On approval, update `Status: approved` and `Branch: plan/<kebab-topic>` together in one atomic file replacement. If the plan has a governing spec, update that spec's `Plan` row to the new plan path in the same approval handoff, then re-read both links before **offering the handoff**: ask whether to kick off the `execute` skill with `<plan-file>` now or later — an offer, never an auto-run.
...
- **Never auto-start.** This skill is invoked deliberately by the human (the `plan` skill). You may *suggest* running it, but never launch it unprompted.
