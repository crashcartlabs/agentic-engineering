...
fixture.** Written 2026-07-04 from recorded real runs, then updated 2026-07-08 for the
resume path: the 2026-06-29 executor dogfood (throwaway repos, verified `a84210e`), the
2026-07-02 typed `/execute` launcher smoke-test (DEVLOG entry of that date), the
2026-07-03 multi-phase reviewer-plan execution (PR #54, credited in #36's closing
comment), and the 2026-07-08 interruption-resume fixture described in Scenario 4.
Nothing below is upgraded past what those records show.

Last verified: 2026-07-08 (resume fixture; typed Claude `/execute` run auth-blocked in
this Codex session)
...
`approved`, on a clean tree (live: `plans/2026-07-02-execute-launcher-smoke.md` in an
...
summary, and the review offer. (Live 2026-07-02: branch
...
offer's name before the 2026-07-03 rename; the current `/review-plan` wording is
...
2026-07-02, separate fresh invocations.)
...
mid-run (live at executor level: an unsatisfiable test in the 2026-06-29 dogfood; a
genuine Phase-4 blocker in the 2026-07-03 reviewer-plan run, PR #54).
...
(Executor-side live twice: 2026-06-29 — stopped, marked `[!]`, amended, refused to
edit the test or use a spoofing hack; 2026-07-03 PR #54 — Phases 1–3 per-phase
commits, real Phase-4 blocker, plan honest across `draft → in-progress`, commits, and
Execution Notes. The **launcher's** blocker relay — surface + no-offer — has not been
observed through a typed `/execute` and stays design-verified.)
...
least one phase has committed but before the plan is `done` (#36's deferred remainder;
also the handoff-named residual). Live fixture: a throwaway repo with
`plans/2026-07-08-resume-live.md` on `plan/resume-live`, status `in-progress`, Phase 1
`[x]`, Phase 2 `[ ]`, and exactly one existing phase commit (`c0f0b41 Phase 1: seed
file`). Negative fixture in the same repo:
`plans/2026-07-08-missing-branch.md`, status `in-progress`, with no
`plan/missing-branch`.
...
honestly, and mismatched branch/status refuses before spawning. (Live 2026-07-08:
...
that exact slash-command path remains auth-blocked in this Codex session.)
