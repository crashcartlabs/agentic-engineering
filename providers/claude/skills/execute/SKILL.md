---
name: execute
description: "Launches or resumes the executor on an approved or in-progress plan. Invoke as /execute <plan-file>. Pre-flights the plan, spawns the executor subagent to implement it autonomously, then relays the result and offers /review-plan. Explicit-trigger only."
...
Invoked as `/execute <plan-file>` (a path under the project's `plans/`).
...
3. **Status + branch agree?** Read the plan's metadata and compute the plan branch:
   ...
   - Any other status = stop with a status-specific message. For `draft`, tell the human
     to approve it first (via `/plan`'s Gate 2). For `done`, say it is already complete.
...
agent, and Pi exposes it through the installed `subagent` extension. If the named agent
...
2. **If it completed:** offer the next handoff — ask whether to kick off `/review-plan <plan-file>` (the plan-conformance reviewer) now or later. An offer, never an auto-run; `/review-plan` is explicit-trigger only.
