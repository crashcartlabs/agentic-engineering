...
Last verified: 2026-07-05 — **Scenario 1 live-verified (2026-07-01 typed run,
transcribed from the DEVLOG record); Scenarios 2–3 design-verified.** The SKILL.md has not changed since
`c38f932` (2026-07-01, the Codex PR #7 fixes), and the `/dogfood handoff` golden run
...
handoff` live run, both skills PASS" (2026-07-01).

## Scenario 1 — Golden: dogfood a registered explicit-trigger skill (live-verified 2026-07-01)

**Input:** In a fresh session, the maintainer typed `/dogfood handoff` — a real,
...
**Verify:** Observed in the 2026-07-01 run. Static validation PASS; invocation-policy
...
**not** prove user-facing `/`-menu registration, which only the maintainer can confirm); live
run — the maintainer typed `/handoff`, artifact landed at
`/tmp/handoff-claude-commit-skill-2026-07-01.md` with the path printed, Workspace
...
for free from the real input; record-keeping stayed an offer until the maintainer approved.
...
**Verify:** Design-verified. The Step-1 stop *predicate* was confirmed on 2026-07-05 —
`skills/definitely-not-a-real-skill/SKILL.md` resolves to no file — but
`dogfood` is explicit-only, so the skill itself could not be implicitly selected in
...
**Verify:** Design-traced only. In the 2026-07-01 golden run the planned distinct
