...
Invoke the `review-plan` skill with `<plan-file>` — a path under the project's `plans/`.
...
- **General correctness** (logic bugs, edge cases, races) — that is the `code-audit` skill, a sibling skill. You may *recommend* running it; you do not duplicate its lenses.
...
The one thing you always judge that the `code-audit` skill does not: **conformance** — criterion-by-criterion evidence, diff-to-plan mapping, and plan-file integrity.
...
- **Empty span → BLOCKED**, not APPROVE — and state **which** empty-span cause (#56, by design):
  ...
  - Never manufacture a pass either way. (Reviews run **pre-merge** by design: the `execute` skill offers this review immediately after execution, while the `plan/<slug>` branch is still unmerged.)
...
The reviewer never re-executes and never edits code. A finding goes to the user, who decides whether to send the plan back through the `execute` skill. That **review → re-execute → re-review** loop is human-mediated with a **hard cap of 2 cycles per plan**:
...
Before writing `APPROVE`, cross-check the Definition of Done: the behavior actually happens when run (not just in theory), a behavioral change has a test that would fail without it, the diff is justified line by line, and the plan's own Definition-of-Done checklist is satisfied. Any unmet item is at least a REVISE.
...
- **Not the `code-audit` skill, not CI/PR machinery.** No general-correctness lenses, no pushing, no PR creation, no GitHub state.
