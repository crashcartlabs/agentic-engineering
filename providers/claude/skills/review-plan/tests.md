...
Scenarios for the `/review-plan` plan-conformance reviewer (born `/review`; renamed at the
2026-07-03 dogfood when the slash menu showed both it and the harness built-in). Each names
the input, the expected verdict/behavior, and how to verify it.

**Status: live-verified** (Phase-4 dogfood, 2026-07-03 — full record at the end of this
...
Last verified: 2026-07-03 (live dogfood on a real `/execute` product; design trace 2026-07-03)
...
unmapped hunk. The skill does not edit code, does not fix either issue, and hands both to the maintainer.
...
never a manufactured PASS — with the cause split per the skill (#56): a branch whose tip is an
...
says "cap reached — handed to the maintainer" rather than running a third automated review.
...
## Phase 4 dogfood — LIVE RECORD (2026-07-03, fresh session, issue #27)

Vehicle: a real `/plan → /execute → /review-plan` pipeline run on
`plans/2026-07-03-plans-placeholder-lint.md` (the maintainer Gate-2-approved; executor agent built it
...
loaded the harness built-in GitHub-PR review (it treated the plan path as a PR ref); the maintainer
confirmed his slash menu showed BOTH entries. Per the skill's own contingency: renamed
...
`plans/2026-07-03-nonexistent.md` → one-line refusal at gate 1; (b) the lint plan while
...
`reviews/2026-07-03-plans-placeholder-lint.md`. All 5 criteria PASS (checker fan-out: 5
...
`Amendments: _None yet._`. Reviewer fixed nothing; the maintainer chose fix + re-review. Note: the
scenario's exact input (unmapped hunk) did not occur — that variant stays design-verified;
the REVISE mechanics (ordering, no-fix, hand-to-the maintainer) are live-verified.
...
`reviews/2026-07-03-plans-placeholder-lint-2.md`.
...
`plans/2026-07-03-reviewer-agent.md` (blocker-scoped acceptance) found its metadata branch
...
(`reviews/2026-07-03-reviewer-agent.md`). Design note surfaced for the maintainer, filed as issue
#56 and resolved same day as by-design (option 1): reviews run pre-merge; the skill's
...
Reports live under `reviews/`; the tracked-reports change makes that directory tracked evidence, so future
