...
never invent a resolution when a side's intent is unclear — plausible-looking code
...
## 2. Understand both sides — read before you write
...
- **A side's intent is unclear** — you cannot tell what one change was *for*, so you cannot
  preserve it. Do not fill the gap with a plausible merge. Lay out both sides and
...
- Change only what is inside the conflict; leave the rest of the file alone — a
...
- **Re-run the repo's gate.** A conflict resolution is a behavioral change — run the
