...
- **Workspace (required, first)** — the **absolute** repo/worktree path, the current branch, and the commit SHA (plus dirty/clean status). The user works across several repos and worktrees — a fresh session must be able to resume in the right checkout; without this, every relative path below is ambiguous and a fresh session can resume in the wrong checkout while believing it's correct.
- **Goal & success criterion** — what we're trying to achieve and how we'll know it's done — the goal needs a success criterion before work counts as done.
