...
don't control (§VIII), so the first question is not "which version" but "should
...
Read `package.json`, the lockfile, and AGENTS.md §VIII before touching anything.
...
Then ask the §VIII question out loud: does the project still need this dep, and
...
A minor/patch that fails the gate is **reverted, not patched over** (§V, §VII — don't
...
- **State applied vs reported every time.** The user never guesses whether the tree
  changed.
- **Adding a new dependency is out of scope** — that's a separate §VIII decision.
