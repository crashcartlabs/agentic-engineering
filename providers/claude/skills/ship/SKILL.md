...
- **A PR already exists for this branch** (`gh pr view --json state,url,number,body`
  ...
  pushing to the branch updates that PR; watching it is the `babysitting-pr` skill's job, not
  a second create.
...
and write the ship-cycle record under `<git-common-dir>` at `ship-self-review/<branch-slug>.md`. Record each run's
