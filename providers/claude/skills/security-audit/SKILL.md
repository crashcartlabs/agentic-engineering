...
argument-hint: "[low|medium|high|max] [--full] [--comment]"
---
...
Invoke the `security-audit` skill with `[effort] [--full] [--comment] [baseline] [paths...]`. Effort defaults to **high**.

This is the exploitability sibling of the `code-audit` skill. `code-audit` asks *"is the code wrong?"*; this asks *"can an attacker make it do something it shouldn't?"* Run both on the same diff — they do not overlap and neither substitutes for the other.
...
- **Correctness, style, performance, tests, structure** — those are the `code-audit` skill, the linter, or sibling skills. A logic bug with no security consequence is not yours.
...
**`--full` skips baselines entirely.** A whole-repository audit needs no base ref,
no merge-base, and no diff: do not run `agentic resolve-base` (a repo with only a
nonstandard branch would fail it for nothing), skip the shared baseline procedure,
declare the **whole repository** as the pinned scope, and state that as what is
under review (see [`--full` mode](#full-mode)). The empty-span stop does not apply.

Otherwise, follow the shared baseline-pinning procedure in
...
`baseline`/`paths...` args, stop if the span is empty, and state what is under
review. The assembled diff is the **pinned scope** — every hunter sees exactly this.
...
- **Stay in scope:** exploitability only. No correctness, style, performance, or test-writing as findings — those are the `code-audit` skill and sibling skills. In diff mode, only what the change introduced or newly exposed; pre-existing vulns go in the trailing note.
