...
Status: **Scenarios 1–2 live-verified; Scenario 3 (clean bill)
and the `--full` / `--comment` paths design-verified.** This was the first live run of
this skill. The golden run drove the full documented pipeline — recon → five parallel
...
## Scenario 1 — Golden: real vuln found, verified, and reported (live-verified)
...
**Verify:** Observed in the live run. Recon mapped the unauthenticated entry → shell sink.
...
it does not become Hardening), this is the boundary case the run surfaced — now
codified in the skill's severity model. Read-only contract held:
the audit changed nothing under review (`git diff HEAD` on the reviewed code was
empty). On report visibility: future reports
...
## Scenario 2 — Edge: empty diff span (live-verified)
...
**Verify:** Observed in the live run — on `main`, the committed (`main...HEAD`),
