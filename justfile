...
build feature repo=invocation_directory():
    {{python}} "{{justfile_directory()}}/scripts/cmux/spawn_fleet.py" \
        ...
        --entry codex:codex:{{codex_model}}:{{quote(feature)}} \
        --entry pi-glm:pi:{{glm_model}}:{{quote(feature)}} \
        --entry pi-minimax:pi:{{minimax_model}}:{{quote(feature)}}
...
debug bug repo=invocation_directory():
    {{python}} "{{justfile_directory()}}/scripts/cmux/spawn_fleet.py" \
        ...
        --entry codex-1:codex:{{codex_model}}:{{quote(bug)}} \
        --entry codex-2:codex:{{codex_model}}:{{quote(bug)}} \
        --entry pi-glm-1:pi:{{glm_model}}:{{quote(bug)}} \
        --entry pi-glm-2:pi:{{glm_model}}:{{quote(bug)}} \
        --entry pi-minimax-1:pi:{{minimax_model}}:{{quote(bug)}} \
        --entry pi-minimax-2:pi:{{minimax_model}}:{{quote(bug)}}
