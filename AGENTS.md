...
**Selftest hygiene.** A selftest must never depend on the probed executable being installed (CI runners lack dev-box binaries — exercise a gate-required tool like `git`, or mock the probe layer) and must isolate ambient environment (`$HERMES_HOME` and similar overrides — pin to a temp dir for the whole run, restore in `finally`). A gate that can write to — or clean up — a real user home is a P1.
...
3. Write and approve a product behavior contract with `spec`; the spec is mandatory
   for product work. The `/bugfix` lane is the explicit, documented exception for
   small, proven fixes — it skips the spec and plan grilling, never the review stack
   (see `skills/bugfix/SKILL.md`).
