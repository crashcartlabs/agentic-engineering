...
# Read bound on the scanned LESSONS.md, kept in sync with the janitor's
# MAX_RECORD_READ_BYTES (1 MiB) so the preview path cannot blow the memory
# budget before the janitor's own bounded reader runs (Codex review note 6).
MAX_LESSONS_READ_BYTES = 1024 * 1024
...
def build_preview(repo: pathlib.Path, *, fetch: bool) -> Preview:
    ...
    if st is None:
        ...
        findings.append(Finding("lessons", "LESSONS.md is not a regular file; refusing to scan."))
    else:
        # Guarded read (same class as the janitor's read_repo_record): a FIFO
        # swapped in after the lstat must not hang the preview (O_NONBLOCK +
        # fstat re-verify), and the read stays bounded at MAX_RECORD_READ_BYTES
        # so an oversized LESSONS.md cannot blow the memory budget before the
        # janitor's own bounded reader runs (Codex review note 6).
        try:
            fd = os.open(
                lessons_path,
                ...
                if not stat.S_ISREG(os.fstat(fd).st_mode):
                    ...
                    while True:
                        ...
                        if not chunk:
                            break
                        data += chunk
                        if len(data) > MAX_LESSONS_READ_BYTES:
                            oversized = True
                            break
                    if oversized:
                        findings.append(
                            Finding("lessons", "LESSONS.md exceeds the size bound; lesson scan skipped.")
                        )
                    else:
                        lesson_candidates, lesson_findings = parse_lessons(data.decode("utf-8"))
                        findings.extend(lesson_findings)
            finally:
                os.close(fd)
        except (OSError, UnicodeDecodeError):
            findings.append(Finding("lessons", "LESSONS.md could not be read safely; refusing to scan."))
