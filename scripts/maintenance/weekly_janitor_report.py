...
# Control characters that may break a GitHub step-summary (notably the bare
# newline, which would end the code span and enable "::" workflow-command
# injection) — neutralized before a filename/detail is rendered. Ordinary
# filename whitespace (spaces AND tabs) is preserved verbatim so two distinct
# candidates ("cache  old.pyc" vs "cache old.pyc", or a tab-separated name)
# stay distinguishable (Codex PR #17 note 6 + round-3 note 6b: tab is a valid
# filename char and must not be collapsed to a space).
_CONTROL_RE = re.compile(r"[\x00-\x08\x0a-\x1f\x7f]")


def _sanitize_report_text(text: str) -> str:
    """Neutralize unsafe control characters but keep spaces and tabs."""
...
MERGE_LIMIT = 10
MAX_RECORD_READ_BYTES = 1024 * 1024  # 1 MiB read bound on repo record files


class RecordReadError(Exception):
    """A repo record file could not be read safely (missing, symlink, non-regular, unreadable, or oversized)."""


def read_repo_record(path: pathlib.Path, *, max_bytes: int = MAX_RECORD_READ_BYTES) -> str:
    """Read a repo-controlled record file with no-follow and a size bound.

    Refuses symlinks (an lstat check plus ``O_NOFOLLOW`` where the platform
    provides it), anything that is not a regular file (directories, FIFOs),
    and files larger than ``max_bytes``; raises RecordReadError so callers
    turn the refusal into a Finding instead of a crash.
    """
    try:
        st = path.lstat()
    except FileNotFoundError:
        raise RecordReadError(f"{path.name} is missing") from None
    except OSError as exc:
        raise RecordReadError(f"{path.name}: {exc}") from exc
    if stat.S_ISLNK(st.st_mode):
        raise RecordReadError(f"{path.name} is a symlink; refusing to read")
    if not stat.S_ISREG(st.st_mode):
        raise RecordReadError(f"{path.name} is not a regular file; refusing to read")
    ...
    try:
        ...
        raise RecordReadError(f"{path.name}: {exc}") from exc
    try:
        # Re-verify after open: the lstat check above leaves a TOCTOU window —
        # a FIFO swapped in between must not block (O_NONBLOCK) and is refused
        # here via fstat.
    ...
    if len(data) > max_bytes:
        raise RecordReadError(f"{path.name} exceeds the {max_bytes}-byte read bound")
    try:
        return data.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise RecordReadError(f"{path.name} is not valid UTF-8: {exc}") from exc
...
def scan_devlog_coverage(
    ...
    *,
    ...
    path = repo / "DEVLOG.md"
    try:
        text = read_repo_record(path)
    except RecordReadError as exc:
        return (
            DevlogCoverage("DEVLOG.md", 0, 0, 0, []),
            [janitor_preview.Finding("devlog", f"{exc}; recent merge coverage was not scanned.")],
        )
    tail, entries_scanned = devlog_tail(text, entry_limit=entry_limit)
    haystack = tail.lower()
    merges: list[MergeCoverage] = []
...
def build_report(
    ...
    *,
    ...
    try:
        lessons_text = read_repo_record(lessons_path)
    except RecordReadError as exc:
        lesson_age_candidates = []
        findings.append(janitor_preview.Finding("lessons", f"{exc}; lesson age scan was skipped."))
    else:
...
def render_markdown(report: WeeklyReport) -> str:
    ...
    if report.lesson_age_candidates:
        for candidate in report.lesson_age_candidates:
            lines.append(
                f"- `LESSONS.md:{candidate.line}` age `{candidate.age_days}` days: {md_code_span(candidate.entry)}"
            )
    if report.absorbed_lesson_candidates:
        for candidate in report.absorbed_lesson_candidates:
            lines.append(
                f"- `LESSONS.md:{candidate.marker_line}-{candidate.lesson_line}` absorbed: {md_code_span(candidate.entry)}\n"
                f"  Human flag reason: {md_code_span(candidate.reason)}"
            )
    ...
    if devlog.merges:
        for merge in missing_merges:
            lines.append(f"- Missing DEVLOG-tail evidence: `{merge.short_sha}` {md_code_span(merge.subject)}")
    ...
    if report.local_branch_candidates:
        for candidate in report.local_branch_candidates:
            subject = f" - {md_code_span(candidate.subject)}" if candidate.subject else ""
    ...
    if issue.action_items:
        for action in issue.action_items:
            ...
            lines.append(
                ...
                f"{md_code_span(action.title)} ({action.url})\n"
                f"  {md_code_span(action.note)}"
            )
    ...
    if issue.dashboard_prs:
        for pr in issue.dashboard_prs:
            ...
            lines.append(
                f"- Dashboard PR #{pr.number}: `{pr.state}`{merged}: {md_code_span(pr.title)} ({pr.url})"
            )
    if issue.stale_open_issues:
        for stale in issue.stale_open_issues:
            labels = f"; labels: {md_code_span(', '.join(stale.labels))}" if stale.labels else ""
            lines.append(
                f"- Stale open issue: #{stale.number} age `{stale.age_days}` days, updated `{stale.updated_at}`"
                f"{labels}: {md_code_span(stale.title)} ({stale.url})"
            )
    else:
    ...
    if report.clutter_candidates:
        for candidate in report.clutter_candidates:
            # Security (security-audit MEDIUM): a hostile filename can carry
            # backticks and newlines — a plain single-backtick span breaks out
            # and injects raw markdown into the step summary. Squash newlines
            # and other unsafe control characters (also kills the "::"
            # workflow-command vector) and use the adaptive fence for both
            # attacker-controlled fields.
            # Codex PR #17 note 6: do NOT collapse ordinary filename
            # whitespace — a candidate like "cache  old.pyc" (repeated spaces)
            # must stay distinct from "cache old.pyc", or the report points
            # cleanup at the wrong path. Only control chars are neutralized;
            # spaces/tabs inside a name are preserved verbatim.
            path = _sanitize_report_text(candidate.path)
            detail = _sanitize_report_text(candidate.detail)
            lines.append(
                f"- {janitor_preview.md_code_span(path)} ({candidate.kind}): "
                f"{janitor_preview.md_code_span(detail)}"
            )
    else:
        lines.append("- None.")
    ...
    if report.findings:
        for finding in report.findings:
            lines.append(f"- `{finding.scope}`: {md_code_span(finding.message)}")
    else:
        lines.append("- None.")
...
def selftest() -> int:
    ...
    record_tmp = pathlib.Path(tempfile.mkdtemp(prefix="weekly-janitor-record-selftest-"))
    try:
        target = record_tmp / "LESSONS.md"
        target.write_text("# LESSONS\n\n- 2026-06-20 - Old enough.\n", encoding="utf-8")
        link = record_tmp / "LESSONS-link.md"
        try:
            link.symlink_to(target)
        except (OSError, NotImplementedError, AttributeError):
            # Symlink creation needs privileges/dev mode on some platforms
            # (Windows runners); skip the assertion where it cannot be set up.
            pass
        else:
            try:
                read_repo_record(link)
                failures.append("record reader followed a symlink")
            except RecordReadError:
                pass
        oversized = record_tmp / "BIG.md"
        oversized.write_text("x" * (MAX_RECORD_READ_BYTES + 1), encoding="utf-8")
        try:
            read_repo_record(oversized)
            failures.append("record reader accepted an oversized file")
        except RecordReadError:
            pass
        plain = read_repo_record(target)
        if not plain.startswith("# LESSONS"):
            failures.append("record reader altered a plain file's contents")
        dir_path = record_tmp / "DEVLOG.md"
        dir_path.mkdir()
        try:
            read_repo_record(dir_path)
            failures.append("record reader accepted a directory")
        except RecordReadError:
            pass
        non_utf8 = record_tmp / "BINARY.md"
        non_utf8.write_bytes(b"# LESSONS\n\xff\xfe binary \x80 bytes\n")
        try:
            read_repo_record(non_utf8)
            failures.append("record reader accepted non-UTF-8 bytes")
        except RecordReadError:
            pass
        fifo = record_tmp / "LESSONS-fifo.md"
        if hasattr(os, "mkfifo"):  # POSIX only; Windows has no FIFOs
            os.mkfifo(fifo)
            try:
                read_repo_record(fifo)
                failures.append("record reader opened a FIFO")
            except RecordReadError:
                pass
            # TOCTOU guard (cycle-2 finding 6): a FIFO that slips past the
            # lstat check must not block — the O_NONBLOCK open returns
            # immediately and the fstat re-verify refuses it.
            try:
                race_fd = os.open(
                    fifo,
                    os.O_RDONLY
                ...
                failures.append(f"O_NONBLOCK open of a FIFO raised: {exc}")
            else:
                if stat.S_ISREG(os.fstat(race_fd).st_mode):
                    failures.append("FIFO fstat re-verify accepted a non-regular file")
                os.close(race_fd)
        locked_parent = record_tmp / "locked"
        locked_parent.mkdir()
        locked_target = locked_parent / "LESSONS.md"
        locked_target.write_text("# LESSONS\n", encoding="utf-8")
        if os.name != "nt" and getattr(os, "geteuid", lambda: -1)() != 0:
            # Windows DAC semantics differ; at euid 0 DAC is bypassed so the
            # read would succeed and the assertion would be a false failure
            # (Codex review note 2). Only assert the permission-denied path
            # where DAC actually applies.
            locked_parent.chmod(0)
            try:
                read_repo_record(locked_target)
                failures.append("record reader read through an unreadable parent")
            except RecordReadError:
                pass
            finally:
                locked_parent.chmod(0o755)  # restore so the temp dir can be removed
    finally:
        shutil.rmtree(record_tmp, ignore_errors=True)

    hostile_title = "**bold** <script>alert(1)</script>"
    hostile_report = dataclasses.replace(
        sample_report,
        issue_report=dataclasses.replace(
            sample_report.issue_report,
            stale_open_issues=[
                StaleIssueCandidate(
                    number=99,
                    ...
                    age_days=18,
                    labels=["<b>urgent</b>"],
                    url="https://github.com/example-org/repo/issues/99",
                )
            ],
        ),
    )
    hostile_rendered = render_markdown(hostile_report)
    if janitor_preview.md_code_span(hostile_title) not in hostile_rendered:
        failures.append("hostile issue title was not escaped into a code span")
    if janitor_preview.md_code_span("<b>urgent</b>") not in hostile_rendered:
        failures.append("hostile label was not escaped into a code span")
    hostile_clutter = dataclasses.replace(
        sample_report,
        clutter_candidates=[
            ClutterCandidate(
                path="a`\n# Fake heading\n- [x] merged `.pyc",
    ...
    hostile_clutter_rendered = render_markdown(hostile_clutter)
    # Note 6: control characters (the embedded newlines) are neutralized
    # before fencing, so the hostile "heading" can never break out as raw
    # markdown regardless of the adaptive fence.
    if "\n# Fake heading\n" in hostile_clutter_rendered:
        failures.append("hostile clutter path injected a raw markdown heading")
    safe_path = janitor_preview.md_code_span("a` # Fake heading - [x] merged `.pyc")
    if safe_path not in hostile_clutter_rendered:
        failures.append("hostile clutter path was not escaped into a code span")
    # Codex PR #17 note 6: a candidate whose filename carries repeated or
    # leading/trailing whitespace must keep that whitespace in the report so
    # cleanup targets the right path; only control chars are neutralized.
    spaced_clutter = dataclasses.replace(
        sample_report,
        clutter_candidates=[
            ClutterCandidate(
                path="cache  old.pyc",  # two spaces — distinct from "cache old.pyc"
                kind="pyc_file",
                detail="5  KiB",  # spaced detail must also survive
            )
        ],
    )
    spaced_rendered = render_markdown(spaced_clutter)
    if "cache  old.pyc" not in spaced_rendered:
        failures.append("clutter filename whitespace was collapsed (note 6)")
    if "5  KiB" not in spaced_rendered:
        failures.append("clutter detail whitespace was collapsed (note 6)")
    # A control character (newline) in the name must still be neutralized
    # into the code span (it must not survive as a literal newline that
    # ends the span and breaks the markdown structure).
    ctrl_clutter = dataclasses.replace(
        sample_report,
        clutter_candidates=[
            ClutterCandidate(path="bad\x0aname.pyc", kind="pyc_file", detail="1 KiB")
        ],
    )
    ctrl_rendered = render_markdown(ctrl_clutter)
    # The neutralized form is "bad name.pyc" (newline -> space). The literal
    # "\x0a" must not appear adjacent to the fenced filename.
    if "bad\x0aname.pyc" in ctrl_rendered:
        failures.append("clutter control character was not neutralized (note 6)")
    if "bad name.pyc" not in ctrl_rendered:
        failures.append("clutter neutralized filename not present (note 6)")
    # Codex PR #17 round-3 note 6b: a TAB in the filename is ordinary
    # whitespace and must be preserved (not collapsed to a space), so a
    # "cache<TAB>old.pyc" candidate stays distinct from "cache old.pyc".
    tab_clutter = dataclasses.replace(
        sample_report,
        clutter_candidates=[
            ClutterCandidate(path="cache\told.pyc", kind="pyc_file", detail="5\tKiB")
        ],
    )
    tab_rendered = render_markdown(tab_clutter)
    if "cache\told.pyc" not in tab_rendered:
        failures.append("clutter filename tab was collapsed to a space (note 6b)")
    if "5\tKiB" not in tab_rendered:
        failures.append("clutter detail tab was collapsed to a space (note 6b)")
