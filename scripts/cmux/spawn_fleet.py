...
def selftest() -> int:
    ...
    state_repo = Path("/tmp/spawn-fleet-selftest-manifest-state")
    ...
    preflight_repo = Path("/tmp/spawn-fleet-selftest-preflight")
    ...
    task_wt = Path("/tmp/spawn-fleet-selftest-task")
    ...
    conflict_wt = Path("/tmp/spawn-fleet-selftest-task-conflict")
    conflict_wt.mkdir(parents=True, exist_ok=True)
    try:
    ...
    symlink_wt = Path("/tmp/spawn-fleet-selftest-task-symlink")
    shutil.rmtree(symlink_wt, ignore_errors=True)
    symlink_wt.mkdir(parents=True)
    ...
    exclude_repo = Path("/tmp/spawn-fleet-selftest-exclude-repo")
    exclude_wt = exclude_repo.parent / "spawn-fleet-selftest-exclude-wt"
    shutil.rmtree(exclude_repo, ignore_errors=True)
    shutil.rmtree(exclude_wt, ignore_errors=True)
    exclude_repo.mkdir(parents=True)
    try:
    ...
    negated_repo = Path("/tmp/spawn-fleet-selftest-negated-gitignore-repo")
    shutil.rmtree(negated_repo, ignore_errors=True)
    negated_repo.mkdir(parents=True)
    try:
    ...
    non_repo_dir = Path("/tmp/spawn-fleet-selftest-not-a-repo")
    ...
    grid_wt = Path("/tmp/spawn-fleet-selftest-grid-task")
    grid_wt.mkdir(parents=True, exist_ok=True)
    try:
    ...
    fleet_repo = Path("/tmp/spawn-fleet-selftest-fleetrepo")
    worktrees_root = fleet_repo.parent / f"{fleet_repo.name}-worktrees"
    stub_dir = Path("/tmp/spawn-fleet-selftest-stubbin")
    ...
    try:
        ...
        ws_fail_stub_dir = Path("/tmp/spawn-fleet-selftest-ws-fail-stubbin")
        shutil.rmtree(ws_fail_stub_dir, ignore_errors=True)
        ws_fail_stub_dir.mkdir(parents=True)
