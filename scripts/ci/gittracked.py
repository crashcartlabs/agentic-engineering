...
def tracked_text_or_none(
    path: pathlib.Path | str,
    **kwargs,
) -> str | None:
    """tracked_text that also returns None on unreadable/undecodable files.

    Text-consuming lints (skill bodies, links, records, plans) must not crash
    the gate on a binary or permission-denied file — they want a finding, not
    a traceback. lint_sediment deliberately keeps using `tracked_text` so its
    latin-1 byte-scan fallback still fires on UnicodeDecodeError.
    """
    try:
        return tracked_text(path, **kwargs)
