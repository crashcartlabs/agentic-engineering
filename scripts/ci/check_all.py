...
SELFTESTS = (
    ...
    (
        "check_upstream.py --selftest",
        REPO / "scripts" / "maintenance" / "check_upstream.py",
        "upstream drift selftest: FAIL",
    ),
...
def run_generate_check() -> int:
    """Run the toolbelt source validation: manifest/version/changelog contracts
    plus generated-adapter drift — the cheap checks quick mode must not skip."""
    proc = subprocess.run(
        [sys.executable, str(REPO / "scripts" / "toolbelt.py"), "check"],
        cwd=REPO,
        capture_output=True,
        text=True,
        env={**os.environ, "PYTHONDONTWRITEBYTECODE": "1"},
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in LINT_MODULES:
        try:
            ...
            print(f"\n{name} crashed: {exc!r}")
    if args.quick:
        # Source validation (manifest sync, version fields, changelog coverage,
        # adapter drift) is otherwise caught only in CI or the slow selftests, so
        # quick mode runs the cheap `toolbelt.py check` directly. Exit tiering
        ...
        try:
            ...
            if generate_status == 1:
                ...
                crashed = True
                print(f"\ntoolbelt check exited {generate_status} (broken check, not a finding)")
