...
2. Identity check: every copy of the shared reference basenames
   (`shared-pipeline.md`, `degraded-delegation.md`) under `skills/*/references/` must
   be byte-identical per basename — those files are single-sourced by convention and
   duplicated only because installed skills must stay self-contained.
...
SHARED_BASENAMES: tuple[str, ...] = ("shared-pipeline.md", "degraded-delegation.md")
...
def shared_file_errors(contents: dict[str, str], basename: str) -> list[str]:
    if len(set(contents.values())) > 1:
        names = ", ".join(sorted(contents))
        return [f"{basename} copies differ ({names}); they must be byte-identical"]
    return []
...
def selftest() -> int:
    ...
    if shared_file_errors({"a": "same", "b": "same"}, "shared-pipeline.md"):
        failures.append("identical shared-pipeline copies were flagged")
    if not shared_file_errors({"a": "same", "b": "different"}, "shared-pipeline.md"):
        failures.append("diverged shared-pipeline copies were accepted")
    if shared_file_errors({"a": "same", "b": "same"}, "degraded-delegation.md"):
        failures.append("identical degraded-delegation copies were flagged")
    if not shared_file_errors({"a": "same", "b": "different"}, "degraded-delegation.md"):
        failures.append("diverged degraded-delegation copies were accepted")
    if not shared_file_errors({"a": "same", "b": "different"}, "shared-pipeline.md"):
        failures.append("diverged shared-pipeline copies were accepted via generalized check")
    import tempfile
...
def main() -> int:
    ...
    shared: dict[str, dict[str, str]] = {basename: {} for basename in SHARED_BASENAMES}
    for path, skip_labels in scan_files():
        ...
        if path.name in SHARED_BASENAMES:
            shared[path.name][rel] = text
    for basename, contents in shared.items():
        errors.extend(shared_file_errors(contents, basename))
