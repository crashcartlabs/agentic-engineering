...
def selftest() -> int:
    ...
    # Codex PR #17 round-4 (P2): an HTML file with invalid UTF-8 (e.g. a 0xff
    # byte) must NOT crash the gate — the HTML loop reads via
    # tracked_text_or_none(), which returns None (reported as a finding) rather
    # than raising UnicodeDecodeError. Pin that the safe reader handles the
    # reported repro instead of throwing.
    import tempfile
    probe = pathlib.Path("docs/_links_selftest_bad.html")
    try:
        probe.write_bytes(b'<a href="x">\xff</a>')
        safe = gittracked.tracked_text_or_none(probe)
        if safe is not None:
            failures.append("invalid-UTF-8 HTML was readable; expected None from safe reader")
    except Exception as exc:  # pragma: no cover - the regression we are guarding against
        failures.append(f"invalid-UTF-8 HTML raised {type(exc).__name__} (should be None): {exc}")
    finally:
    ...
    if failures:
        ...
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("markdown link selftest: OK (local, external, fenced, and invalid-UTF-8 HTML negatives pinned)")
    return 0
...
def main() -> int:
    ...
    for md in iter_md(tracked):
        source_text = gittracked.tracked_text_or_none(md)
    ...
    for page in sorted(path for path in tracked if path.suffix.lower() in {".html", ".htm"}):
        # Codex PR #17 round-4 (P2): use the safe reader like the Markdown
        # path above — tracked_text() raises UnicodeDecodeError on invalid
        # UTF-8 (reproduced with a 0xff byte in docs/cmux-guide.html) and
        # aborts the whole gate; tracked_text_or_none() returns None so the
        # file is reported as a lint finding instead of crashing.
        source_text = gittracked.tracked_text_or_none(page)
        if source_text is None:
