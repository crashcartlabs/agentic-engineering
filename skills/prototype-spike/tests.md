...
**Status: Scenarios 1–3 all live-verified.** S1 (golden) ran first on
pipeline-dashboard viability. S2 (decline) and S3 (promotion refusal) ran in a later
session across three more typed invocations; the dogfood captures below hold the details.

All three scenarios were exercised as typed live runs.
...
the codebase can't answer (the SDK is a third-party binary). (Live: "can
...
## Dogfood record — first typed run (S1 live)

Invoked in a fresh session on a real uncertainty: *"Can there be a dashboard that
...
- **Stays in lane:** offered `/plan` with the conclusion as prior art; the conclusion
  was parked rather than planned immediately — a deliberate deferral, which the skill
  allowed without starting to build.

## Dogfood record — S2 decline (two live declines)
...
- *"add a GSD-style research phase to planning"* (with resource links) — a subtler
  ...
  apparatus), so no experiment was warranted. Declined and parked as a follow-up issue.
...
## Dogfood record — S3 promotion refusal (live)
