...
description: "Safety scan of an agent skill before it lands in `skills/` — reads the skill's SKILL.md plus its references/assets and flags prompt-injection/instruction-override, exfiltration, and unsafe tool use, so a malicious or careless skill cannot be promoted unreviewed. Invoke the `skill-safety-scan` skill with `<skill-name|path>`. Read-only: writes a report, never edits the scanned skill. Not credential-scanning (CI does that), not code-correctness (the `code-audit` skill), not an OWASP code audit (the `security-audit` skill). Explicit-trigger only."
---
...
Invoke the `skill-safety-scan` skill with `<target>`, where `<target>` is a skill name (resolved to `skills/<name>/`) or a path to a skill directory.

You **scan directly**. The surface is one skill's handful of markdown/script files — a small, closed set — so there is no subagent fan-out and no trust-boundary map. That machinery is the `security-audit` skill's, for an application's attack surface; here it would be over-built — write the minimum that solves the problem in front of you.
...
- **Code correctness** — logic, edge cases, whether the skill's own scripts actually work. That is the `code-audit` skill.
- **Exploitable vulnerabilities in application code** — SQLi, XSS, SSRF, the OWASP classes. That is the `security-audit` skill.
...
Migration from the pre-tracked-report flow: if `<git-common-dir>/info/exclude` (via `git rev-parse --git-common-dir`) contains an exact stale `skill-scans/` or `/skill-scans/` line, remove just that line before writing the report; if a broader local pattern would still hide `skill-scans/`, refuse and name the offending pattern — a scan report hidden from `git status` is not durable evidence.
...
- **Stay in scope:** skill *content* safety only. Not credentials-in-repo (CI secret-scan), not code correctness (the `code-audit` skill), not application vulnerabilities (the `security-audit` skill).
