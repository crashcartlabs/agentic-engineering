...
Scenarios captured from **real pre-codification runs** (2026-07-04): four live ships
in this repo (PRs #57, #58, #59, #61 — the /build-skill practice corpus) and staged
refusal runs in a scratch repo (`shiplab`). The first **typed** `/ship` run landed
later the same day (fresh session, PR #63): Scenario 1's legacy gate/push/Codex path
...
remain verified by the manual/staged runs. Full typed-run record in #60's closing
comment.

Last verified: 2026-07-08 (S1 typed-run live; S2–S5 manual/staged runs; S6
self-review gate dogfooded on the internal bake-off branches)
...
existing PR (live: `claude/item9-friction-cuts` → PR #61, and three more that day).
...
**Status:** Live-verified via the first typed run, 2026-07-04 — PR #63 (evidence SHA
...
`/babysitting-pr 63` pointer printed, **shipped** stated. One deviation, recorded on
#60: the local scanner was missing → installed by a named route (brew, same version
CI uses) rather than CI's pipe-to-shell installer. The internal self-review fields
did not exist yet and are not claimed as PR #63-verified.
...
`BLOCKED: evidence stale; not shipped` (traced from the 2026-07-04 #57 flow, where
follow-up pushes updated the open PR and no second create was attempted);
(e) detached HEAD (live-staged 2026-07-04: `git checkout $(git rev-parse HEAD)`,
...
(formatter, regenerated snapshot, lockfile) during the run (live-staged 2026-07-04:
...
after roughly 5 shipped PRs, revisit this skill's self-review checklist.
...
**Status:** Dogfooded on the internal cmux bake-off branches, 2026-07-08. The internal
