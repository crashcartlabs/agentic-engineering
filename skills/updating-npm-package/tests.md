...
Last traced: live run, scratch repo, real registry

All four scenarios ran live against the real npm registry in a scratch
...
**Status: split.** The major-detection → not-applied path and the
breaking-changes enumeration are **live-verified** (see the run below). The *concrete
...
## Verification record (live — scratch repo, real npm registry)
