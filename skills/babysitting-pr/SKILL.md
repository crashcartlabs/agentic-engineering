...
Read before you act: pull the PR's current **status** (open/merged/closed),
...
tool independently: without `subscribe_pr_activity`, use the harness's persistent
watch/poll primitive for PR-state deltas (Claude Code: `Monitor`); without
`send_later`, use its scheduled-wake primitive for an hourly check-in (Claude Code:
`CronCreate`). Never duplicate a native capability with its fallback; tear down whatever
