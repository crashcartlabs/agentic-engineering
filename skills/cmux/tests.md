...
**`codex` substitution note:** `codex` is not installed on the machine this
was verified on (confirmed: `which codex` finds nothing). Every scenario
...
flags, does it actually build anything) has never been observed on real
hardware and remains unverified.
...
actually hit in this session's runs.
...
generated worktree paths) is out of scope for this plan — flagged here for
...
The final whole-branch review (after all 6 tasks individually passed) caught
...
## Bugs found and fixed (Codex PR review, PR #85)
...
## Bugs found and fixed (Codex PR review round 2, follow-up to PR #85)
...
installed cmux (follow-up PR, branch `fix/cmux-fleet-preflight-gaps`):
...
## Bug found and fixed (Codex PR review round 3, follow-up to PR #86)
